...
Update terbaru:
- Menambahkan troubleshooting Breeze/Vite untuk error `Could not resolve './bootstrap'`.
- Menambahkan langkah perbaikan jika `resources/js/bootstrap.js` terisi command PowerShell.
...
# TROUBLESHOOTING A — Breeze/Vite Error `resources/js/bootstrap.js`
...
```text
[UNRESOLVED_IMPORT] Could not resolve './bootstrap' in resources/js/app.js
```

Artinya file ini dicari oleh Vite:

```text
resources/js/bootstrap.js
```

tetapi file tersebut belum ada atau hilang.
...
Isi manual di VSCode dengan kode berikut:
...
## Jika File `bootstrap.js` Terlanjur Salah Isi

Kadang command PowerShell bisa tidak sengaja masuk ke dalam file, misalnya isi file menjadi seperti ini:
...
```text
Set-Content -Path .\resources\js\bootstrap.js ...
```

Jika terjadi, Vite akan error seperti:
...
```powershell
Remove-Item .\resources\js\bootstrap.js -Force
New-Item -ItemType File .\resources\js\bootstrap.js
code .\resources\js\bootstrap.js
```

Lalu isi manual hanya dengan:
...
```powershell
Get-Content .\resources\js\bootstrap.js
```

Output yang benar harus hanya:
...
Masalah ini bukan kegagalan Laravel atau Breeze.

Penyebabnya hanya:

```text
resources/js/app.js mengimpor ./bootstrap,
tetapi file resources/js/bootstrap.js tidak ada atau isinya bukan JavaScript valid.
```
...
```text
[ ] resources/js/bootstrap.js ada dan berisi JavaScript valid
